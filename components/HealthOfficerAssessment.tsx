...
const HealthOfficerAssessment: React.FC<Props> = ({ worker, onSave, onCancel }) => {
  ...
  const ppeOptions = [
    { 
        ...
        activeBorder: 'border-emerald-300 dark:border-emerald-500/50',
    ...
    { 
        ...
        activeBorder: 'border-red-300 dark:border-red-500/50',
        dotColor: 'bg-red-500',
        Icon: XCircle 
    }
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white dark:bg-slate-800/50 p-8 rounded-2xl border border-slate-200 dark:border-white/10 animate-in fade-in zoom-in-95 duration-300 shadow-xl dark:shadow-none">
      <div className="flex items-center gap-3 mb-6 border-b border-slate-200 dark:border-white/10 pb-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">فرم ارزیابی سلامت شغلی و ایمنی</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm">کارشناس بهداشت حرفه‌ای: {worker.name} - {worker.department}</p>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-xl border border-slate-200 dark:border-white/5">
          <div className="flex justify-between items-center mb-4">
              <h3 className="text-slate-900 dark:text-white font-bold text-lg">عوامل زیان‌آور محیط کار</h3>
              <div className="flex gap-2">
                  <button onClick={() => handleBulkAction('none')} className="text-xs flex items-center gap-1 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 px-3 py-1.5 rounded-lg text-slate-600 dark:text-slate-300 transition-colors border border-slate-200 dark:border-transparent">
                      <Square className="w-3 h-3" /> هیچکدام
                  </button>
                  <button onClick={() => handleBulkAction('all')} className="text-xs flex items-center gap-1 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 px-3 py-1.5 rounded-lg text-slate-600 dark:text-slate-300 transition-colors border border-slate-200 dark:border-transparent">
          ...
          <div className="grid md:grid-cols-2 gap-6">
              {Object.entries(HAZARD_CATEGORIES).map(([category, items]) => (
                  <div key={category} className="space-y-2">
                      <h4 className="text-cyan-700 dark:text-cyan-400 text-sm font-bold border-b border-slate-200 dark:border-white/5 pb-1 mb-2">{category}</h4>
                      <div className="grid grid-cols-2 gap-2">
                        {items.map((item) => (
                        <label key={item.k} className={`flex items-center gap-2 p-2 rounded-lg border transition-all cursor-pointer ${data.hazards[item.k] ? 'bg-cyan-50 dark:bg-cyan-900/40 border-cyan-300 dark:border-cyan-500/50' : 'bg-white dark:bg-slate-800 border-transparent hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
                            <input 
                            type="checkbox" 
                            checked={!!data.hazards[item.k]} 
                            onChange={() => handleHazardToggle(item.k)}
                            className="w-4 h-4 rounded border-slate-400 dark:border-slate-500 text-cyan-500 focus:ring-cyan-500"
                            />
                            <span className="text-xs text-slate-700 dark:text-slate-200">{item.l}</span>
                        </label>
                        ))}
                      </div>
                  </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
           <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-white/5">
              <h3 className="text-slate-900 dark:text-white font-bold mb-3">وضعیت استفاده از PPE</h3>
                    ...
                    return (
                        <button
                            ...
                            className={`
                                ...
                                ${isSelected 
                                    ? `${opt.activeBg} ${opt.activeBorder} ${opt.activeColor} shadow-sm` 
                                    : 'bg-white dark:bg-slate-800 border-transparent text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-750'}
                            `}
                        >
           ...
           <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-white/5 flex flex-col justify-center">
              <label className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all cursor-pointer h-full ${data.needsDoctorVisit ? 'bg-red-50 dark:bg-red-500/10 border-red-500 animate-pulse' : 'bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 hover:border-slate-400 dark:hover:border-slate-500'}`}>
                 <input 
                    type="checkbox" 
                 ...
                 />
                 <div>
                    <span className={`block font-bold text-lg mb-1 ${data.needsDoctorVisit ? 'text-red-600 dark:text-red-400' : 'text-slate-600 dark:text-slate-300'}`}>ارجاع به پزشک طب کار</span>
        ...
        <div>
            <div className="flex justify-between items-center mb-2">
                <label className="text-slate-600 dark:text-slate-300 text-sm">توضیحات تکمیلی کارشناس</label>
                <button 
          ...
          <textarea 
            className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-slate-900 dark:text-white h-32 focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all placeholder-slate-400 dark:placeholder-slate-600 leading-relaxed"
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-white/10">
          <button onClick={onCancel} className="px-6 py-3 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-white rounded-xl transition-colors">
            انصراف
          </button>
