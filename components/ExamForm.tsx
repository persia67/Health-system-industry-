...
const ExamForm: React.FC<Props> = ({ initialData, workerName, onSubmit, onCancel }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-white/10 flex flex-col h-[calc(100vh-100px)]">
      {/* Form Header */}
      <div className="bg-slate-100 dark:bg-slate-800 p-4 border-b border-slate-200 dark:border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex-1 w-full">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2 mb-2">
                ...
                فرم معاینات سلامت شغلی (پرونده پزشکی)
            </h2>
            <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input 
                        ...
                        className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded px-2 py-1 font-mono text-slate-800 dark:text-slate-200 w-32 focus:border-cyan-500 outline-none"
                        placeholder="وارد کنید..."
                        disabled={!!workerName} // Disable if worker is known
                    />
                </div>
            </div>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
            <button onClick={onCancel} className="flex-1 md:flex-none px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700">انصراف</button>
                            ...
                            <div key={item.id} className={`p-4 rounded-xl border transition-colors ${item.hasCondition ? 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-500/30' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-white/5'}`}>
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        <label className="font-medium text-slate-900 dark:text-white cursor-pointer" onClick={() => handleHistoryChange(item.id, 'hasCondition', !item.hasCondition)}>
                                            {item.question}
                                        </label>
                                        {item.hasCondition && (
                                            <input 
                                                ...
                                                className="mt-3 w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg p-2 text-sm focus:border-red-500 outline-none"
                                            />
                                        )}
                                    </div>
                                </div>
                    ...
                    <div className="flex justify-between items-center border-b pb-2">
                        ...
                        <div className="flex items-center gap-2">
                             ...
                             <input 
                                ...
                                className="w-24 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1 text-center font-mono"
                             />
                        </div>
                    </div>
                                ...
                                <div key={key} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl p-4 shadow-sm">
                                    ...
                                    <div className="space-y-4">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {config.symptoms.map(s => (
                                                    <button 
                                                        ...
                                                        className={`text-xs px-2 py-1 rounded border transition-colors ${systemData.symptoms.includes(s) ? 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-300 border-red-300 dark:border-red-500/30' : 'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600'}`}
                                                    >
                                                        {s}
                                                    </button>
                                                ))}
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {config.signs.map(s => (
                                                    <button 
                                                        ...
                                                        className={`text-xs px-2 py-1 rounded border transition-colors ${systemData.signs.includes(s) ? 'bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 border-amber-300 dark:border-amber-500/30' : 'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-600'}`}
                                                    >
                                                        {s}
                                                    </button>
                                                ))}
                                        ...
                                        <textarea 
                                            ...
                                            className="w-full text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-lg p-2 min-h-[60px]"
                                        />
                                    </div>
                                </div>
                            );
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200 dark:border-white/5">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="number" step="0.01" value={formData.spirometry.fvc} onChange={(e) => updateState('spirometry', {...formData.spirometry, fvc: parseFloat(e.target.value)})} className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-center" />
                            </div>
                            <div>
                                <label className="text-xs text-slate-500 block mb-1">FEV1 (Lit)</label>
                                <input type="number" step="0.01" value={formData.spirometry.fev1} onChange={(e) => updateState('spirometry', {...formData.spirometry, fev1: parseFloat(e.target.value)})} className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-center" />
                            </div>
                             <div>
                                <label className="text-xs text-slate-500 block mb-1">Ratio %</label>
                                <input type="number" value={formData.spirometry.fev1_fvc} onChange={(e) => updateState('spirometry', {...formData.spirometry, fev1_fvc: parseFloat(e.target.value)})} className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-center" />
                            </div>
                            <div>
                                ...
                                <select 
                                    ...
                                    className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-center text-sm"
                                >
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200 dark:border-white/5">
                                        ...
                                        {formData.hearing.left.map((val, idx) => (
                                            <td key={idx} className="p-1">
                                                <input 
                                                    ...
                                                    className="w-12 p-1 text-center border rounded focus:border-blue-500 outline-none bg-slate-50 dark:bg-slate-800"
                                                />
                                            </td>
                                        ))}
                                    </tr>
                                        ...
                                        {formData.hearing.right.map((val, idx) => (
                                            <td key={idx} className="p-1">
                                                <input 
                                                    ...
                                                    className="w-12 p-1 text-center border rounded focus:border-red-500 outline-none bg-slate-50 dark:bg-slate-800"
                                                />
                                            </td>
                                        ))}
                                    </tr>
                        ...
                        <textarea 
                            ...
                            className="w-full mt-4 p-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-sm"
                        ></textarea>
                    </div>
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div className="space-y-2">
                                            ...
                                            <input placeholder="Uncorrected" value={formData.vision.acuity.right.uncorrected} onChange={(e) => updateState('vision', {...formData.vision, acuity: {...formData.vision!.acuity, right: {...formData.vision!.acuity.right, uncorrected: e.target.value}}})} className="w-full p-2 border rounded text-center bg-white dark:bg-slate-900" />
                                            <input placeholder="Corrected" value={formData.vision.acuity.right.corrected} onChange={(e) => updateState('vision', {...formData.vision, acuity: {...formData.vision!.acuity, right: {...formData.vision!.acuity.right, corrected: e.target.value}}})} className="w-full p-2 border rounded text-center bg-white dark:bg-slate-900" />
                                        </div>
                                        <div className="space-y-2">
                                            <div className="font-bold text-center mb-1">چشم چپ (OS)</div>
                                            <input placeholder="Uncorrected" value={formData.vision.acuity.left.uncorrected} onChange={(e) => updateState('vision', {...formData.vision, acuity: {...formData.vision!.acuity, left: {...formData.vision!.acuity.left, uncorrected: e.target.value}}})} className="w-full p-2 border rounded text-center bg-white dark:bg-slate-900" />
                                            <input placeholder="Corrected" value={formData.vision.acuity.left.corrected} onChange={(e) => updateState('vision', {...formData.vision, acuity: {...formData.vision!.acuity, left: {...formData.vision!.acuity.left, corrected: e.target.value}}})} className="w-full p-2 border rounded text-center bg-white dark:bg-slate-900" />
                                        </div>
                                    </div>
                                </div>
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <select value={formData.vision.colorVision} onChange={(e) => updateState('vision', {...formData.vision, colorVision: e.target.value})} className="w-full p-2 border rounded bg-white dark:bg-slate-900"><option>Normal</option><option>Abnormal</option></select>
                                    </div>
                                    <div>
                                        <label className="text-xs text-slate-500 block mb-1">عمق دید (Depth)</label>
                                        <input value={formData.vision.depthPerception} onChange={(e) => updateState('vision', {...formData.vision, depthPerception: e.target.value})} className="w-full p-2 border rounded bg-white dark:bg-slate-900" />
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            {['wbc', 'rbc', 'hb', 'plt', 'fbs', 'chol', 'tg', 'creatinine', 'alt', 'ast'].map(test => (
                                <div key={test}>
                                    ...
                                    <input 
                                        ...
                                        className="w-full p-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-center font-mono"
                                    />
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="space-y-4 bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200 dark:border-white/5">
                        <div>
                            ...
                            <textarea 
                                ...
                                className="w-full h-32 p-4 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900"
                         ...
                         <div>
                            ...
                            <textarea 
                                ...
                                className="w-full h-20 p-4 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900"
